self.onmessage = function(e) {
    ...
    const baseValue4 = data.value4 || 50;
    const baseValue5 = data.value5 || 50;
    ...
    for (let i = 0; i < totalImageCount; i++) {
        ...
        const horizontalTilt = (value2 - 50) / 50;
        const verticalTilt = (value3 - 50) / 50;
        const zoomFactor = 1 + (value4 - 50) / 100;
        const rotation = (value5 - 50) / 50 * Math.PI;
...
function getRandomValue(baseValue, index, seed) {
    ...
    // Generate a random value between -100 and 100
    const randomOffset = (random() - 0.5) * 200;
...
function applyPerspective(x, y, width, height, strength, horizontalTilt, verticalTilt, zoomFactor, rotation) {
    ...
    // Apply perspective
    let px = nx / (1 - ny * strength * 2);
    let py = ny / (1 - nx * strength * 2);

    // Apply tilt
    px += horizontalTilt * ny * 2;
    py += verticalTilt * nx * 2;
