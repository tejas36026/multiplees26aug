...
document.addEventListener("presentationstart", (e) => {
    ...
    for (let word in lexicons) {
      if (Array.isArray(lexicons[word])) {
        if (
          lexicons[word].includes("Conjunction") ||
          lexicons[word].includes("Preposition") ||
          lexicons[word].includes("Pronoun")
        ) {
          stopwords.push(word);
        }
      } else {
        if (
          lexicons[word].match("Conjunction") ||
          lexicons[word].match("Preposition") ||
          lexicons[word].match("Pronoun")
        ) {
          stopwords.push(word);
        }
      }
    ...
    let added_nums = coordinates
      ? [].concat.apply(
          [],
          coordinates.map((coord) => coord.match(/\d+/g))
        )
      : [];
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      for (let term of terms) {
        if (term.tags && term.tags.includes("Value")) {
          ...
          let next_token =
            i < tokens.length - 1 ? tokens[i + 1].text.toLowerCase() : "";
          let total_token = prev_token + " " + next_token;
  
          if (
            ["x", "left", "corner"].some((word) => total_token.includes(word)) ||
            (total_token.includes("sides") &&
              doc.text().includes("quadrilateral"))
          ) {
            x_values.push(num);
            x++;
            // console.log(`Added ${num} to x_values: ${x_values}`);
          } else if (
            ["y", "top"].some((word) => total_token.includes(word)) ||
            (total_token.includes("sides") &&
              doc.text().includes("quadrilateral"))
          ) {
            y_values.push(num);
            ...
            ["right", "rhs"].some((word) => total_token.includes(word))
          ) {
            x_values.push(num);
          } else if (
            ["bottom", "down"].some((word) => total_token.includes(word))
          ) {
            y_values.push(num);
          }
        }
    ...
    if (x + y + z + w < value_count && value_count < 4) {
      ...
      if (values.length >= 2) {
        if (x_values.length === 0 && y_values.length === 0) {
          ...
          if (height_values.length === 0) {
            height_values.push(values[1]);
          }
          console.log(
            `Assumed width: ${values[width_values.length - 1]}, height: ${
              values[height_values.length - 1]
            }`
          );
    ...
    const messageInput = document.querySelector("#message-input").value;
      ...
      const baseUrl = "https://ciasimbaya-objectdetection-anushreep774.glitch.me";
          const imageUrl = "https://cdn.glitch.global/cec98c17-c614-43a2-9170-5c58f1cd9bfa/image%20-%202024-05-13T185904.005.jpg?v=1716742194080";
          const endpoints = [
              "/caption",
              "/image_segmentation",
              "/generate_image",
          ...
          async function createImageFormData(endpoint) {
              const formData = new FormData();
              if (endpoint === "/laionclip" || endpoint === "/openaiclip") {
                  ...
                  formData.append("candidate_labels", "label1,label2,label3"); // Add your own labels here
              } else {
                  const imageBlob = await fetchImageAsBlob(imageUrl);
          ...
          function createDummyJsonData() {
              return JSON.stringify({ prompt: "A description of the image" });
          }
          ...
          async function makeRequest(endpoint) {
              ...
              if (endpoint.includes("classify") || endpoint.includes("direction") || endpoint.includes("generate_image")) {
                  options = {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: createDummyJsonData()
                  };
              } else {
                  const formData = await createImageFormData(endpoint);
                  options = {
                      method: "POST",
                      body: formData
                  };
              ...
              if (endpoint === "/laionclip" || endpoint === "/openaiclip") {
                  document.getElementById('result-container').innerHTML = '<div class="spinner"></div>';
              }
  
              try {
                  const response = await fetch(url, options);
                  return `${endpoint}: ${response.ok ? "Success" : "Failed"}`;
              } catch (error) {
          ...
          async function runAllRequests() {
              ...
              resultsDiv.innerHTML = "Running requests...";
    ...
    function handleInputChange() {
      ...
      // console.log('image :>> ', image);
      let imageUrls = [];
      let imageUrl = `https://source.unsplash.com/600x500/?${encodeURIComponent(
        messageInput
      )}`;
      ...
      function Invert() {
        if (!canApplyFilter) {
          console.log(
            "Cannot apply or remove filter within 5 seconds of last operation"
          );
          return;
        }
      ...
      function Blur() {
        ...
        const array = window.shared.imageEditorArray;
        var imageEditor =
          window.shared.imageEditorArray[
            window.shared.imageEditorArray.length - 1
          ];
        setTimeout(function () {
          imageEditor
            .addShape("rect", {
      ...
      function Brightness() {
        setTimeout(function () {
          ...
          let imageEditor = array[0];
          setTimeout(function () {
            let brightnessValue = bright_num
              ? parseInt(bright_num, 10) / 255
              : 70;
      ...
      function Textfn() {
        setTimeout(() => {
          ...
          let fontWeight =
            typeof font_Weight1 !== "undefined" ? font_Weight1 : "normal";
          // // alert(fontWeight)
      ...
      function Saturation() {
        ...
        setTimeout(() => {
          let saturationValue = saturation_num
            ? parseInt(saturation_num, 10)
            : 50;
      ...
      function Mask() {
        ...
        setTimeout(() => {
          ...
          async function loadImageFromUrl() {
            ...
            var url =
              "https://source.unsplash.com/500x600/?" +
      ...
      function dataextraction() {
        ...
        for (let i = 0; i < tokens.length; i++) {
          ...
          for (let term of terms) {
            if (term.tags && term.tags.includes("Value")) {
              ...
              if (
                ["x", "left", "corner"].some((word) =>
                  total_token.includes(word)
                ) ||
                (total_token.includes("sides") &&
                  doc.text().includes("quadrilateral"))
              ) {
        ...
        console.log(
          `x: ${x_values}, y: ${y_values}, width: ${width_values}, height: ${height_values}`
        );
        ...
        console.log(
          `x: ${x_values[0] || 0}, y: ${y_values[0] || 0}, width: ${
            width_values[0] || 100
          }, height: ${height_values[0] || 100}`
        );
          ...
          for (let term of terms) {
            if (term.tags && term.tags.includes("Value")) {
              ...
              if (
                ...
                [" y", "y ", "top"].some((word) => total_token.includes(word))
              ) {
                y_values.push(num);
          ...
          if (shape_name === "rectangle" || shape_name === "rect") {
            ...
            imageEditor
              .addShape("circle", {
                ...
                rx: Math.min(
                  500,
                  !isNaN(extractNumber1(value_x)) &&
                    typeof value_x !== "undefined" &&
                    value_x !== null
                    ? extractNumber1(value_x)
                    : 100
                ),
                ry: Math.min(
                  500,
                  !isNaN(extractNumber1(value_y)) &&
                    typeof value_y !== "undefined" &&
                    value_y !== null
                    ? extractNumber1(value_y)
                    : 100
                ),
                isRegular: false,
              })
      ...
      function setDrawingShapecircle() {
        ...
        imageEditor.setDrawingShape("circle", {
          // When resizing, the shape keep the 1:1 ratio
          rx: 10,
          ry: 10,
          isRegular: true,
        });
      ...
      function ink() {
        ...
        setTimeout(() => {
          ...
          imageEditor
            .loadImageFromURL(glfxDataUrl, "SampleImage2")
            .then((sizeValue) => {
      ...
      function tiltShift_blur() {
        ...
        setTimeout(() => {
          ...
          canvas_glfx
            .draw(texture_glfx)
            .tiltShift(96, 359.25, 480, 287.4, 26, 271)
            .update();
      ...
      function lensBlur() {
        ...
        setTimeout(() => {
          ...
          canvas_glfx
            .draw(texture_glfx)
            .lensBlur(
              extractsixnumber[0] || 11,
              extractsixnumber[1] || 0.05,
              extractsixnumber[2] || 0.45841
            )
            .update();
      ...
      function bulgePinch() {
        ...
        setTimeout(() => {
          ...
          canvas_glfx
            .draw(texture_glfx)
            .bulgePinch(
              extractsixnumber[0] || 320,
              extractsixnumber[1] || 239.5,
              extractsixnumber[2] || 600,
              extractsixnumber[3] || 0.34
            )
            .update();
        ...
        setTimeout(() => {
          ...
          canvas_glfx
            ...
            .perspective(
              ...
              [175, 156, 496, 55, 173, 336, 504, 330]
            )
            .update();
      ...
      function edgeWork() {
        ...
        setTimeout(() => {
          ...
          canvas_glfx
            .draw(texture_glfx)
            .hexagonalPixelate(320, 239.5, 10)
            .update();
          //        canvas_glfx.draw(texture_glfx).dotScreen(320, 239.5, 0.59, 4.87).update();
          //        canvas_glfx.draw(texture_glfx_glfx).colorHalftone(320, 239.5, 0.64, 3).update();
        ...
        setTimeout(() => {
          ...
          canvas_glfx
            ...
            .dotScreen(
              extractsixnumber[0] || 320,
              extractsixnumber[1] || 239.5,
              extractsixnumber[2] || 0.59,
              extractsixnumber[3] || 4.87
            )
            .update();
          //  canvas_glfx.draw(texture_glfx_glfx).colorHalftone(320, 239.5, 0.64, 3).update();
        ...
        setTimeout(() => {
          ...
          canvas_glfx
            ...
            .colorHalftone(
              extractsixnumber[0] || 320,
              extractsixnumber[1] || 239.5,
              extractsixnumber[2] || 0.64,
              extractsixnumber[3] || 3
            )
            .update();
          image_glfx.parentNode.insertBefore(canvas_glfx, image_glfx);
          image_glfx.parentNode.removeChild(image_glfx);
      ...
      function cloth() {
        ...
        async function sendImageToServer(originalImageData) {
          ...
          try {
            const response = await fetch(
              "https://ciasimbaya-objectdetection-anushreep774.glitch.me/image_segmentation",
              {
                method: "POST",
                body: formData,
              }
            ...
            if (response.ok) {
              ...
              Object.entries(segmentationData).forEach(
                ([label, result], index) => {
        ...
        canvas.on("mouse:move", (options) => {
          ...
          if (
            ...
            const label = Object.entries(labelToColor).find(
              ([_, value]) => value === closestColorString
            )?.[0];
            console.log(
              `Closest color: ${closestColorString}, Label: ${label || "Unknown"}`
            );
  ...
  //       try {
  //         const response = await fetch(
  //           "https://ciasimbaya-objectdetection-anushreep774.glitch.me/generate_image",
  //           {
  //             method: "POST",
  //             headers: {
  ...
  //       imageEditor
  //         .loadImageFromURL(imageUrls[imageUrls.length - 1], "SampleImage2")
  //         .then((sizeValue) => {
  //           imageEditor.ui.activeMenuEvent();
  //           imageEditor.ui.resizeEditor({ imageSize: sizeValue });
  //           console.log("Image allegedly loaded.");
  ...
  async function text_to_image(retryCount = 0) {
    const prompt = document.querySelector("#message-input").value;
    const imageContainer = document.getElementById("imageid");
    ...
    try {
      ...
      if (data.error && data.error.includes("Model") && data.error.includes("loading")) {
        ...
        resultElement.textContent = `Model is loading. Estimated time: ${estimatedTime} seconds. Retrying...`;
        
        // Retry after the estimated time
        setTimeout(() => text_to_image(retryCount + 1), estimatedTime * 1000);
        return;
      }
      ...
      await imageEditor.loadImageFromURL(imageUrls[imageUrls.length - 1], "SampleImage2");
      imageEditor.ui.activeMenuEvent();
      imageEditor.ui.resizeEditor({ imageSize: imageEditor.getCanvasSize() });
      console.log("Image loaded successfully.");
      resultElement.textContent = "Image generated and loaded successfully.";
  
    } catch (error) {
      console.error("Error:", error);
      if (retryCount < 3) {
        const backoffTime = Math.pow(2, retryCount) * 1000;
        resultElement.textContent = `Error occurred. Retrying in ${backoffTime/1000} seconds...`;
        setTimeout(() => text_to_image(retryCount + 1), backoffTime);
      } else {
        resultElement.textContent = `Error: ${error.message}. Max retries reached.`;
      }
    }
  }
  ...
  async function sendImageToServer(originalImageData) {
    ...
    try {
      ...
      async function classify(payload) {
        try {
          const response = await fetch(
            "https://ciasimbaya-objectdetection-anushreep774.glitch.me/classify",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
      ...
      function displayClassificationResult(data) {
        ...
        if (data.labels && data.scores) {
          ...
          const greaterTopBottom =
            topBottom === scores[topIndex] ? "top" : "bottom";
          const greaterLeftRight =
            leftRight === scores[leftIndex] ? "left" : "right";
          greaterTopBottomValue = greaterTopBottom;
          greaterLeftRightValue = greaterLeftRight;
          console.log(
            `Greater value for top/bottom: ${topBottom} (${greaterTopBottom})`
          );
          console.log(
            `Greater value for left/right: ${leftRight} (${greaterLeftRight})`
          );
          resultDiv.textContent = JSON.stringify(data, null, 2);
        } else {
          resultDiv.textContent =
            "Labels and scores not found in the response data.";
        }
      }
      ...
      async function classify() {
        ...
        try {
          ...
          const horizontalLabel =
            maxHorizontalScore === leftScore ? "left" : "right";
  
          console.log(
            `The vertical label with the highest score is: ${verticalLabel}`
          );
          console.log(
            `The horizoal label with the highest score is: ${horizontalLabel}`
          );
          ...
          try {
            ...
            const directionResponse = await fetch(
              "https://ciasimbaya-objectdetection-anushreep774.glitch.me/direction",
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
          ...
          try {
            const directionResponse1 = await fetch(
              "https://ciasimbaya-objectdetection-anushreep774.glitch.me/direction",
              {
                method: "POST",
                headers: {
   ...
   function laionclip() {
    ...
    function sendImageToServer(candidateLabels, retryCount) {
      ...
      document.getElementById('result-container').innerHTML = '<div class="text-center"><div class="inline-block animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-gray-900"></div></div>';
      ...
      fetch("https://ciasimbaya-objectdetection-anushreep774.glitch.me/laionclip", {
        ...
        .then((data) => {
          ...
          if (data.error && data.error.includes("Model") && data.error.includes("loading")) {
            const estimatedTime = data.estimated_time || 20;
            document.getElementById('result-container').innerHTML = `<p>Model is loading. Retrying in ${estimatedTime} seconds...</p>`;
        ...
        .catch((error) => {
          ...
          if (retryCount < 3) {
            const backoffTime = Math.pow(2, retryCount) * 1000;
            document.getElementById('result-container').innerHTML = `<p>Error occurred. Retrying in ${backoffTime/1000} seconds...</p>`;
            setTimeout(() => sendImageToServer(candidateLabels, retryCount + 1), backoffTime);
          } else {
            document.getElementById('result-container').innerHTML = `<p>Error: ${error.message}. Max retries reached.</p>`;
          }
        });
    }
  ...
  function openaiclip() {
    ...
    function showModal() {
      let modal = document.getElementById('openai-modal');
      if (!modal) {
        modal = document.createElement('div');
      ...
      modal.innerHTML = `
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
          <h3 class="text-lg font-semibold mb-4">OpenAI CLIP Labels</h3>
          <div id="labels-container" class="mb-4"></div>
          <div class="flex mb-4">
            <input type="text" id="label-input" class="flex-grow mr-2 p-2 border rounded" placeholder="Enter a label">
            <button id="add-label" class="bg-blue-500 text-white px-4 py-2 rounded">Add</button>
    ...
    showModal();
      ...
      function face() {
        ...
        async function sendImageToServer(originalImageData) {
          ...
          try {
            const response = await fetch(
              "https://ciasimbaya-objectdetection-anushreep774.glitch.me/faces",
              {
                method: "POST",
                body: formData,
              }
  ...
  function ocr() {
    ...
    if (!modal) {
      ...
      modalTitle.textContent = "OCR Results";
      
      const modalBody = document.createElement("div");
      modalBody.id = "ocr-modal-body";
    ...
    function displayInModal(text, isRetry = false) {
      const modalBody = document.getElementById("ocr-modal-body");
      const resultDiv = document.createElement("div");
      resultDiv.innerHTML = `<h3>${isRetry ? "Retry Result" : "Initial Result"}</h3><pre>${text}</pre>`;
  ...
  async function sendImageToServer(originalImageData) {
      ...
      const url = "https://ciasimbaya-objectdetection-anushreep774.glitch.me/ocr";
      const maxRetries = 5; // Increased from 3 to 5
      let retries = 0;
  
      while (retries < maxRetries) {
        try {
          displayInModal(`Attempt ${retries + 1}: Sending request to server...`, true);
          ...
          if (data.error && data.error.includes("Model is currently loading")) {
            const waitTime = 20; // 20 seconds wait time
            displayInModal(`Model is loading. Waiting ${waitTime} seconds before retry attempt ${retries + 1}...`, true);
            await new Promise((resolve) => setTimeout(resolve, waitTime * 1000));
            retries++;
          } else if (Array.isArray(data) && data[0] && data[0].generated_text) {
            console.log("OCR successful");
            displayInModal(data[0].generated_text, retries > 0);
            return;
          } else {
          ...
          displayInModal(`Error: ${error.message}. Waiting 20 seconds before retry attempt ${retries + 1}...`, true);
          await new Promise((resolve) => setTimeout(resolve, 20000)); // 20 seconds wait
          retries++;
          if (retries >= maxRetries) {
            console.error("Max retries reached. OCR failed.");
            displayInModal("OCR failed after 5 attempts. Please try again later.", true);
          }
        }
      }
    }
  ...
  function image_classification() {
    ...
    function initImageEditor() {
      ...
      setTimeout(() => sendImageToServer(0), 300);
    }
      ...
      modal.innerHTML = `
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <h3 class="text-xl font-semibold text-gray-900">Image Classification</h3>
    ...
    function sendImageToServer(retryCount) {
      ...
      showModal('<div class="flex justify-center items-center"><div class="animate-spin rounded-full h-10 w-10 border-b-2 border-gray-900"></div></div>');
      ...
      fetch("https://ciasimbaya-objectdetection-anushreep774.glitch.me/image_classification", {
        ...
        .then((data) => {
          ...
          if (data.text && typeof data.text === 'object' && data.text.error) {
            // Handle the case where text is an object containing an error
            const errorMessage = data.text.error;
            const estimatedTime = data.text.estimated_time || 20;
            showModal(`
              <p>Error: ${errorMessage}</p>
              <p>Estimated time: ${estimatedTime} seconds. Retrying...</p>
            `);
            setTimeout(() => sendImageToServer(retryCount + 1), estimatedTime * 1000);
          } else if (data.text && Array.isArray(data.text)) {
  
            const classList = data.text.map((item, index) => {
              const label = item.label || 'Unknown';
              const score = item.score || 0;
              return `<li>${index + 1}. Class: ${label}, Confidence: ${(score * 100).toFixed(2)}%</li>`;
            }).join('');
            showModal(`
              <p class="font-bold mb-2">Image Classification Results:</p>
            ...
            showModal(`
              ...
              <pre class="mt-2 bg-gray-100 p-2 rounded">${JSON.stringify(data, null, 2)}</pre>
            `);
        ...
        .catch((error) => {
          ...
          if (retryCount < 3) {
            const backoffTime = Math.pow(2, retryCount) * 1000;
            showModal(`<p>Error occurred. Retrying in ${backoffTime/1000} seconds...</p>`);
            setTimeout(() => sendImageToServer(retryCount + 1), backoffTime);
          } else {
            showModal(`
              <p>Error: ${error.message}. Max retries reached.</p>
              <p class="mt-2">Stack trace:</p>
              <pre class="mt-2 bg-gray-100 p-2 rounded text-xs">${error.stack}</pre>
            `);
          }
        });
    }
  ...
  function sendImageToServer(retryCount) {
    ...
    fetch("https://ciasimbaya-objectdetection-anushreep774.glitch.me/looks", {
      ...
      .then((data) => {
        ...
        if (data.error && data.error.includes("Model") && data.error.includes("loading")) {
          const estimatedTime = data.estimated_time || 20;
          showModal(`<p>Model is loading. Estimated time: ${estimatedTime} seconds. Retrying...</p>`);
          setTimeout(() => sendImageToServer(retryCount + 1), estimatedTime * 1000);
        } else if (data.error) {
          showModal(`<p>Error: ${data.error}</p>`);
        } else {
          // Handle the array response
          if (Array.isArray(data) && data.length === 2) {
            const aestheticScore = parseFloat(data[0]);
            const nonAestheticScore = parseFloat(data[1]);
            
            if (!isNaN(aestheticScore) && !isNaN(nonAestheticScore)) {
              const resultHtml = `
                <p>Aesthetic: ${(aestheticScore * 100).toFixed(2)}%</p>
                <p>Non-Aesthetic: ${(nonAestheticScore * 100).toFixed(2)}%</p>
              `;
              showModal(`<h4>Aesthetic Analysis:</h4>${resultHtml}`);
            } else {
              showModal(`<p>Invalid scores received: ${JSON.stringify(data)}</p>`);
            }
          } else {
            showModal(`<p>Unexpected response format: ${JSON.stringify(data)}</p>`);
          }
        }
      })
      .catch((error) => {
        if (retryCount < 3) {
          ...
          showModal(`<p>Error: ${error}. Max retries reached.</p>`);
        }
      });
  }
        initImageEditor();
      ...
      function f() {
        ...
        function sendImageToServer() {
          ...
          fetch(
            "https://ciasimbaya-objectdetection-anushreep774.glitch.me/human_or_artificial",
            {
              method: "POST",
              body: formData,
            }
            ...
            .then((data) => {
              ...
              if (data.error) {
                ...
                document.getElementById(
                  "result"
                ).textContent = `Classification: ${data.text}`;
              }
            })
            .catch((error) => {
  ...
  function detect_face_express() {
    ...
    function showModal(content) {
      ...
      modal.innerHTML = `
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <h3 class="text-xl font-semibold text-gray-900">Face Expression Detection</h3>
            <button id="close-modal" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
              <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </button>
          </div>
  ...
  function sendImageToServer(retryCount) {
    ...
    fetch("https://ciasimbaya-objectdetection-anushreep774.glitch.me/detect_face_express", {
      ...
      .then((data) => {
        ...
        if (data.error && data.error.includes("Model") && data.error.includes("loading")) {
          ...
          const expressionList = data.result.map(item => 
            `<li>${item.label}: ${(item.score * 100).toFixed(2)}%</li>`
          ).join('');
          
          showModal(`
            <p>Face Expression Analysis:</p>
    ...
    function showModal(content) {
      ...
      modal.innerHTML = `
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <h3 class="text-xl font-semibold text-gray-900">Image Quality Analysis</h3>
            <button id="close-modal" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
              <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </button>
          </div>
  ...
  function sendImageToServer(retryCount) {
    ...
    fetch("https://ciasimbaya-objectdetection-anushreep774.glitch.me/quality_how_lq_or_hq", {
      ...
      .then((data) => {
        console.log("data :>> ", data);
        if (data.text && data.text.error && data.text.error.includes("Model") && data.text.error.includes("loading")) {
          const estimatedTime = data.text.estimated_time || 20;
          showModal(`<p>Model is loading. Estimated time: ${estimatedTime} seconds. Retrying...</p>`);
          setTimeout(() => sendImageToServer(retryCount + 1), estimatedTime * 1000);
        } else if (data.error) {
          ...
          const qualityList = data.map(item => 
            `<li>${item.label}: ${(item.score * 100).toFixed(2)}%</li>`
          ).join('');
          ...
          // Sort the results by score in descending order
          data.text.sort((a, b) => b.score - a.score);
          
          const qualityList = data.text.map(item => 
            `<li>${item.label}: ${(item.score * 100).toFixed(2)}%</li>`
          ).join('');
  ...
  function face_happy_sad() {
    ...
    function showModal(content) {
      ...
      modal.innerHTML = `
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <h3 class="text-xl font-semibold text-gray-900">Emotion Analysis Result</h3>
            <button id="close-modal" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
              <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </button>
          </div>
    ...
    fetch("https://ciasimbaya-objectdetection-anushreep774.glitch.me/face_happy_sad", {
      ...
      .then((data) => {
        ...
        if (data.error && data.error.includes("Model") && data.error.includes("loading")) {
          ...
          // Handle the array of emotion objects
          const emotionList = data.text.map(emotion => {
            return `<li>${emotion.label}: ${(emotion.score * 100).toFixed(2)}%</li>`;
          }).join('');
      ...
      fetch(
        ...
        .then((data) => {
          ...
          if (data.error && data.error.includes("Model") && data.error.includes("loading")) {
            ...
            showModal(`<p>OCR Result: ${JSON.stringify(data)}</p>`);
          }
        })
        .catch((error) => {
          if (retryCount < 3) {
  ...
  function age() {
    ...
    function showModal(content) {
      ...
      modal.innerHTML = `
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <h3 class="text-xl font-semibold text-gray-900">Age Detection Result</h3>
            <button id="close-modal" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
              <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </button>
          </div>
      ...
      fetch("https://ciasimbaya-objectdetection-anushreep774.glitch.me/age", {
        ...
        .then((data) => {
          ...
          if (data.error && data.error.includes("Model") && data.error.includes("loading")) {
            ...
            showModal(`<p>Age Result: ${JSON.stringify(data)}</p>`);
          }
        })
        .catch((error) => {
          if (retryCount < 3) {
  ...
  function human_or_artificial() {
    ...
    function showModal(content) {
      ...
      modal.innerHTML = `
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <h3 class="text-xl font-semibold text-gray-900">Human or Artificial Result</h3>
            <button id="close-modal" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
              <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </button>
          </div>
   ...
   function sendImageToServer(retryCount) {
      ...
      fetch("https://ciasimbaya-objectdetection-anushreep774.glitch.me/human_or_artificial", {
        ...
        .then((data) => {
          ...
          if (data.text && data.text.error && data.text.error.includes("Model") && data.text.error.includes("loading")) {
            ...
            // Handle the case where data.text is an array of objects
            if (Array.isArray(data.text)) {
              const resultHtml = data.text.map(item => {
                return `<p>${item.label}: ${(item.score * 100).toFixed(2)}%</p>`;
              }).join('');
              showModal(`<h4>Classification Results:</h4>${resultHtml}`);
            } else {
              showModal(`<p>Classification: ${JSON.stringify(data.text)}</p>`);
            }
          }
        })
        .catch((error) => {
    ...
    modal.innerHTML = `
      <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="flex items-center justify-between p-4 border-b border-gray-200">
          <h3 class="text-xl font-semibold text-gray-900">Aesthetic Analysis Result</h3>
          <button id="close-modal" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
          </button>
        </div>
  ...
  if (Array.isArray(data) && data.length === 2) {
    const resultHtml = `
      <p>Aesthetic: ${JSON.stringify(data[0])}</p>
      <p>Non-Aesthetic: ${JSON.stringify(data[1])}</p>
    `;
    showModal(`<h4>Aesthetic Analysis:</h4>${resultHtml}`);
  } else {
    showModal(`<p>Unexpected response format: ${JSON.stringify(data)}</p>`);
    ...
    initImageEditor();
      ...
      function room_scene() {
        ...
        async function sendImageToServer(originalImageData) {
          ...
          try {
            const response = await fetch(
              "https://ciasimbaya-objectdetection-anushreep774.glitch.me/scenes",
              {
                method: "POST",
                body: formData,
              }
  ...
  //       try {
  //         const response = await fetch(
  //           "https://ciasimbaya-objectdetection-anushreep774.glitch.me/caption",
  //           {
  //             method: "POST",
  //             body: imageData,
  //           }
  ...
  //       }
  //     }
    async function queryImageCaptioning(imageData, maxRetries = 3) {
    let retries = 0;
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (data.error && data.error.includes("Model is currently loading")) {
          const timeout = data.estimated_time ? data.estimated_time * 1000 : 20000; // Default to 20 seconds if no time is provided
          console.log(`Image captioning model is loading. Waiting for ${timeout / 1000} seconds...`);
          await new Promise(resolve => setTimeout(resolve, timeout));
          retries++;
          console.log(`Retrying image captioning (Attempt ${retries} of ${maxRetries})...`);
        } else {
          return data.text; // Successfully got the caption
        }
      } catch (error) {
        console.error("Error generating image caption:", error);
        retries++;
        if (retries >= maxRetries) {
          throw new Error(`Failed to generate image caption after ${maxRetries} attempts`);
        }
      }
    }
  }
  ...
  function displayImageCaptionResult(caption) {
    ...
    let modal = document.getElementById('caption-modal');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'caption-modal';
      document.body.appendChild(modal);
    }
    ...
    window.addEventListener('click', (event) => {
      ...
      async function queryObjectDetection(imageData) {
        try {
          const response = await fetch(
            "https://ciasimbaya-objectdetection-anushreep774.glitch.me/detect",
            {
              method: "POST",
              body: imageData,
            }
  ...
  //         try {
  //           // Use Function.prototype.apply.call to ensure 'this' is correct
  ...
  //             originalMethod(...args);
  //           } catch (innerError) {
  //             console.error(`Error calling console.${property}:`, innerError);
  //           }
  //         }
  ...
  //                   arg && arg.constructor ? arg.constructor.name : typeof arg
  //                 }]`;
  //               }
  //             })
  //             .join(" ");
  //           // Get the current script name, simplified
  ...
  //             "unknown";
  //           consoleOutput.push(
  //             `[${property.toUpperCase()}] [${scriptName}] ${output}`
  //           );
  //           isLogging = false;
  //         }
  //       };
  ...
  //           })
  //           .join(" ");
  //         consoleOutput.push(`[${method.toUpperCase()}] ${output}`);
  ...
  //       let dialog = document.querySelector('[data-dialog="dialog-xl"]');
  ...
  //         backdrop.setAttribute("data-dialog-backdrop", "dialog-xl");
  //         backdrop.setAttribute("data-dialog-backdrop-close", "true");
  //         backdrop.className =
  //           "fixed inset-0 z-[999] grid h-screen w-screen place-items-center bg-black bg-opacity-60 transition-opacity duration-300";
  
  //         dialog = document.createElement("div");
  //         dialog.setAttribute("data-dialog", "dialog-xl");
  //         dialog.className =
  //           "relative m-4 w-3/4 min-w-[75%] max-w-[75%] rounded-lg bg-white  bg-opacity-20 font-sans text-base font-light leading-relaxed text-blue-gray-500 antialiased shadow-2xl";
  ...
  //           log.includes("imageeditorafterimage.js")
  //         );
  //         if (relevantLogs.length > 0) {
  //           loadingIndicator.style.display = "none";
  
  //           const lastFiveLogs = relevantLogs.slice(-5); // Only keep the last 5 logs
  //           const logElements = lastFiveLogs
  //             .map((log) => {
  //               const [type, filename, ...message] = log.split("] ");
  ...
  //                 .replace(/\(https?:\/\/[^)]+\)/, "")
  //                 .trim();
  //               // return `<p class="text-sm text-shadow-neon ${getLogTypeClass(type)} ">[${timestamp}] ${escapeHtml(cleanMessage)}</p>`;
  //               return `<p class="text-sm font-bold  ${getLogTypeClass(
  //                 type
  //               )}  ${getNeonClass(type)}">[${timestamp}] ${escapeHtml(
  //                 cleanMessage
  //               )}</p>`;
  //             })
  //             .join("");
  
  //           modalContent.innerHTML = logElements;
  //           modalContent.scrollTop = modalContent.scrollHeight;
  //         } else {
  //           loadingIndicator.style.display = "flex";
  //           modalContent.innerHTML =
  //             '<p class="text-sm text-gray-500">No logs from imageeditorafterimage.js yet.</p>';
  //         }
  //       }
  
  //       function getNeonClass(type) {
  //         switch (type) {
  //           case "[ERROR":
  //             return "text-red-500";
  //           case "[WARN":
  ...
  //       const updateInterval = setInterval(updateModalContent, 1000);
  ...
  //     }
      ...
      const keywordsActions = [
        ...
        ["human_or_artificial", human_or_artificial],
      ...
      async function executeActions(messageInput, keywordsActions) {
        for (let i = 0; i < keywordsActions.length; i++) {
          ...
          if (messageInput.includes(keyword)) {
            ...
            //                  const functionName = functionNames[action.name] || keyword;
  
            //             // Show the execution modal
            //             const modalElement = showExecutionModal(functionName);
