    ...
    function ocr() {
          ...
          function content_in_img() {
            ...
            function sendImageToServer() {
              ...
              // Send the POST request to the server
              fetch('https://ciasimbaya-objectdetection-anushreep774.glitch.me/content_in_img', {
                method: 'POST',
                body: formData,
              })
                .then(response => response.json())
...
//                     } catch (error) {
//                         return `[Complex object: ${arg && arg.constructor ? arg.constructor.name : typeof arg}]`;
//                     }
//                 }).join(' ');
                
//                 consoleOutput.push(`[${property.toUpperCase()}] ${output}`);
//             };
//         }
    ...
    get: function(target, property) {
        ...
        if (typeof originalMethod === 'function') {
            return function(...args) {
                ...
                // Capture the output, but only if we're not already logging
                if (!isLogging) {
                    isLogging = true;
                    const output = args.map(arg => {
                        try {
                    ...
                    const scriptName = stack.split('\n')[2]?.match(/(?:^|\/)([^/]+\.js)/)?.[1] || 'unknown';
                    
                    consoleOutput.push(`[${property.toUpperCase()}] [${scriptName}] ${output}`);
                    isLogging = false;
                }
            };
        }
...
function openmodal() {
    ...
    let backdrop = document.querySelector('[data-dialog-backdrop="dialog-xl"]');
    ...
    if (!dialog) {
        ...
        backdrop.className = 'fixed inset-0 z-[999] grid h-screen w-screen place-items-center bg-black bg-opacity-60 transition-opacity duration-300';

        dialog = document.createElement('div');
        dialog.setAttribute('data-dialog', 'dialog-xl');
        dialog.className = 'relative m-4 w-3/4 min-w-[75%] max-w-[75%] rounded-lg bg-white font-sans text-base font-light leading-relaxed text-blue-gray-500 antialiased shadow-2xl';
        ...
        dialog.innerHTML = `
            ...
            <div id="modal-content" class="p-4 space-y-2 max-h-[60vh] overflow-y-auto"></div>
        `;
...
function updateModalContent() {
    const relevantLogs = consoleOutput.filter(log => log.includes('imageeditorafterimage.js'));
    if (relevantLogs.length > lastLogCount) {

        const newLogs = relevantLogs.slice(lastLogCount);
        const newLogElements = newLogs.map(log => {
            const [type, filename, ...message] = log.split('] ');
            const timestamp = new Date().toISOString().split('T')[1].split('.')[0];
            // Remove any remaining file path or line number information
            const cleanMessage = message.join('] ').replace(/\(https?:\/\/[^)]+\)/, '').trim();
            return `<p class="text-sm ${getLogTypeClass(type)}">[${timestamp}] ${escapeHtml(cleanMessage)}</p>`;
        }).join('');
        modalContent.insertAdjacentHTML('beforeend', newLogElements);
        modalContent.scrollTop = modalContent.scrollHeight;
        lastLogCount = relevantLogs.length;
    }
}
    // Initial update
    updateModalContent();
