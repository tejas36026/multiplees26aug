self.onmessage = function(e) {
    ...
    for (let i = 0; i < imageCount; i++) {
        ...
        const hueShift = (i / (imageCount - 1)) * 360;
        ...
        for (const region of selectedRegions) {
            for (const pixelIndex of region) {
                ...
                if (l < 0.2) {
                    l = Math.min(1, l + 0.4); // Significant increase for very dark pixels
                } else if (l < 0.5) {
                    l = Math.min(1, l + 0.3); // Moderate increase for medium-dark pixels
                }

                // More pronounced saturation increase
                s = Math.min(1, s * 2); // Double the saturation
