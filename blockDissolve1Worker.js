self.onmessage = function(e) {
    ...
    for (let i = 0; i < imageCount; i++) {
        ...
        // Apply block dissolve effect with increased movement to selected regions
        for (let y = 0; y < height; y += blockSize) {
            for (let x = 0; x < width; x += blockSize) {
                ...
                if (blockSelected && Math.random() < dissolveThreshold) {
                    // Calculate dynamic offset based on position and current frame
                    const angleOffset = (x / width + y / height + i / imageCount) * Math.PI * 2;
