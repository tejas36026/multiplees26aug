...
function segmentImage(imageData, threshold = 30) {
    ...
    const hueThreshold = 25; // in degrees
    const saturationThreshold = 0.25;
    const valueThreshold = 0.35;

    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const index = y * width + x;
            if (!visited[index]) {
                const segment = floodFill(data, segmentedData, width, height, x, y, thresholdSquared, hueThreshold, saturationThreshold, valueThreshold, visited);
...
function floodFill(data, segmentedData, width, height, x, y, thresholdSquared, hueThreshold, saturationThreshold, valueThreshold, visited) {
    const stack = [[x, y]];
    const segment = [];
    ...
    while (stack.length) {
        ...
        if (isColorSimilarHybrid(baseColor, currentColor, baseHSV, currentHSV, thresholdSquared, hueThreshold, saturationThreshold, valueThreshold)) {
            segment.push(index);
...
function isColorSimilarHybrid(rgb1, rgb2, hsv1, hsv2, thresholdSquared, hueThreshold, saturationThreshold, valueThreshold) {
